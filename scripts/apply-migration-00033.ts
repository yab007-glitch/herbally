...
const KNOWN_EXCEPTIONS: Record<string, { po: boolean; pt: boolean; no: boolean; nt: boolean }> = {
...
async function main() {
  ...
  // Get all herbs
  const herbList: Array<{ id: string; slug: string; pregnancy_safe: boolean | null; nursing_safe: boolean | null }> = [];
  let page = 0;
  while (true) {
  ...
  for (const herb of herbList) {
    ...
    if (exception) {
      ...
      if (!error) exceptions++;
      if (error && exceptions <= 3) console.error(`  ❌ ${herb.slug}: ${error.message}`);
    } else {
      ...
      if (!error) updated++;
      if (error && updated <= 3) console.error(`  ❌ ${herb.slug}: ${error.message}`);
    }
  ...
  if (aloe) {
    ...
    console.log(`  Pregnancy oral: ${aloe.pregnancy_safe_oral}, topical: ${aloe.pregnancy_safe_topical}`);
    console.log(`  Nursing oral: ${aloe.nursing_safe_oral}, topical: ${aloe.nursing_safe_topical}`);
  }
}
