...
async function main() {
  ...
  console.log(
    `Processing ${herbs.length} herbs (${dryRun ? "DRY RUN" : "LIVE"})...\n`
  );
  ...
  for (const herb of herbs as HerbRow[]) {
    ...
    if (existingProvenance?.verification_method === "manual") {
      alreadyVerified++;
      skipped++;
      continue;
    }
  ...
  console.log(
    `  Skipped:    ${skipped} (${alreadyVerified} already manually verified)`
  );
