...
function parseJsonbBuildArray(sql: string): Array<{ slug: string; citations: unknown[] }> {
  const results: Array<{ slug: string; citations: unknown[] }> = [];
  
  // Match blocks: SET citations = jsonb_build_array(...) WHERE slug = 'xxx'
  const blockRegex = /SET citations = jsonb_build_array\(([\s\S]*?)\)\s*WHERE slug = '([a-z0-9-]+)'/g;
  let match;
...
function parseJsonStringFormat(sql: string): Array<{ slug: string; citations: unknown[] }> {
  const results: Array<{ slug: string; citations: unknown[] }> = [];
...
async function main() {
  ...
  const sql = fs.readFileSync("supabase/migrations/00031_add_pubmed_citations.sql", "utf-8");
  ...
  const keySlugs = ["turmeric", "ashwagandha", "echinacea", "ginkgo-biloba", "lavender", "peppermint", "green-tea", "milk-thistle", "valerian"];
