...
const seedContent = fs.readFileSync(
  "supabase/migrations/00015_seed_herbs.sql",
  "utf-8"
);
    ...
    if (!inString) {
      if (ch === "(") {
        ...
        if (depth === 0) {
          ...
          while (
            j + 1 < valuesSection.length &&
            /[\s,]/.test(valuesSection[j + 1])
          ) {
            j++;
          }
        }
...
fs.writeFileSync(
  "supabase/migrations/00029_add_missing_seed_herbs.sql",
  output
);
