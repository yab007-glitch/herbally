 ...
 *
 * Usage:
 *   npx tsx scripts/generate-herb-faqs.ts [--concurrency=5] [--model=glm-5]
 */
...
interface Progress {
  completed: Record<string, string>; // slug -> timestamp
...
- Each answer must be 40-80 words, evidence-based, and directly answer the question
...
- Output STRICT JSON only: { "faqs": [{"question":"...","answer":"...","category":"general|safety|dosage|interactions|mechanism"}] }
...
Generate 4-6 FAQ pairs. Output as JSON.`;
}
...
async function processHerb(
  ...
  try {
    ...
    const rows = faqs.map((faq, index) => ({
      ...
      is_featured: index < 2, // Top 2 FAQs are featured
      sort_order: index,
    }));
...
async function runBatch() {
  ...
  for (let i = 0; i < pending.length; i += CONCURRENCY) {
    ...
    console.log(`Progress: ${completed}/${allHerbs.length} completed, ${failed} failed (${((completed / allHerbs.length) * 100).toFixed(1)}%)`);
  }
