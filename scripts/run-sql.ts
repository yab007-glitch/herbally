...
async function main() {
  ...
  const sql = fs.readFileSync("supabase/migrations/00033_dual_safety_flags.sql", "utf-8");
  ...
  try {
    ...
    console.log(`   SQL endpoint: ${res.status} ${await res.text().then(t => t.substring(0, 100))}`);
  } catch (err) {
