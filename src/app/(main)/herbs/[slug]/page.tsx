...
export default async function HerbDetailPage({ params }: Props) {
  ...
  const isUnverified = provenance.verification_method === "unverified";
  ...
  if (dbMonograph) {
    ...
    // For unverified herbs, override the fabricated mechanism text with
    // an honest disclosure. The stored DB monographs were AI-generated with
    // specific molecular pathway claims (e.g. "modulates NF-κB, COX-2") that
    // were never verified against primary literature. Don't present them as fact.
    if (isUnverified && monograph.mechanism) {
    ...
    if (isUnverified && monograph.summary) {
