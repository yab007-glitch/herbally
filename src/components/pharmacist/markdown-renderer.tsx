...
const SEVERITY_STYLES: Record<InteractionSeverity, { label: string; bg: string; text: string; border: string }> = {
  contraindicated: {
...
const components: Components = {
  a({ href, title, children, ...rest }: ComponentPropsWithoutRef<"a">) {
    ...
    return (
      ...
      >
        {children}
        {isExternal && <ExternalLink className="ml-0.5 inline size-3 align-baseline" aria-hidden="true" />}
      </a>
    );
  },
  ...
  p({ children, node, ...rest }: ComponentPropsWithoutRef<"p"> & { node?: unknown }) {
    const props = ((node as { properties?: Record<string, string> } | undefined)?.properties ?? {}) as Record<string, string>;
  ...
  strong({ children, node, ...rest }: ComponentPropsWithoutRef<"strong"> & { node?: unknown }) {
    const props = ((node as { properties?: Record<string, string> } | undefined)?.properties ?? {}) as Record<string, string>;
